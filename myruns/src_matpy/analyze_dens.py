...
for bio_indx in range(len(biom_arr)): # loop in biomass
    ...
    for pdi_val in pdi_arr:
        ...
        for tval in range(temp_min,temp_max,temp_dt): # loop in temp
            ...
            yrho_avg = np.append(ytemp_avg,yavg/len(run_arr)) #end of tval loop
        
        # Save density-time plots
        ax1.legend(loc=0)
        fig1.savefig(fig_dir+'/'+'denstime_'+str(pdi_val)+'.png',dpi=fig1.dpi)
        plt.close(fig1)
        ...
        for t_plt in range(temp_min,temp_max,temp_dt): # loop in temp
            ...
            fc_tg.write('%s\t%g\t%g\n' %(biomass,t_plt,1000.0/yrho_avg[indx]))
            indx += 1
        fc_tg.close()

        # Plot SV-temperature data
        df=pd.read_table(tg_fyl)
        figa, axa = plot_tg(df,fig_dir,pdi_val) 
        tgval = compute_tg(df,axa) #compute tgval
        figa.savefig(fig_dir+'/'+'sv2_'+str(pdi_val)+'.png',dpi=figa.dpi)
        figa.savefig(fig_dir+'/'+'sv2_'+str(pdi_val)+'.eps',format='eps')
    ...
    fig2.savefig(fig_dir + '/'+'sv_all.eps',format='eps')
    plt.close(fig2)
#------------------------------------------------------------------
