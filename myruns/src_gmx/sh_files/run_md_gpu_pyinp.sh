...
if ! test -f "$fnvthigh_inp"; then
	...
	srun gmx_mpi grompp -f nvt_high.mdp -c confout_min.gro -p py_topol -n tcgrp_indx.ndx -o nvt_high.tpr 
	wait
...
if ! test -f "$fnvt_inp"; then
        ...
        srun gmx_mpi grompp -f nvt.mdp -c confout_nvt_high.gro -p py_topol -n tcgrp_indx.ndx -o nvt.tpr
        wait
...
if ! test -f "$fnpt_berend_inp"; then
	...
	srun gmx_mpi grompp -f npt_berendsen.mdp -c confout_nvt.gro -p py_topol -n tcgrp_indx.ndx -o npt_berendsen.tpr
	wait
...
if ! test -f "$fnpt_inp"; then
	...
	srun gmx_mpi grompp -f npt_main.mdp -c confout_npt_berendsen.gro -p py_topol -n tcgrp_indx.ndx -o npt_main.tpr
	wait
